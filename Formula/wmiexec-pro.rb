class WmiexecPro < Formula
  include Language::Python::Virtualenv

  desc "New generation of wmiexec.py"
  homepage "https://github.com/XiaoliChan/wmiexec-Pro"
  url "https://github.com/XiaoliChan/wmiexec-Pro/archive/refs/tags/" \
      "v0.4.1.tar.gz"
  sha256 "f21b03ac8aad5fddc0779510266c95d905651f8ac55d9893529e533bfcfe5077"
  ...
  def install
    inreplace "requirements.txt", /^numpy.*\n/, ""

    inreplace "lib/modules/rid_hijack.py" do |s|
      s.gsub!("\nimport numpy", "")
      s.sub!(
        "from impacket.dcerpc.v5.dtypes import NULL\n\n" \
        "class RID_Hijack_Toolkit():",
        "from impacket.dcerpc.v5.dtypes import NULL\n\n" \
        "def split_into_chunks(values, chunk_size=4):\n    " \
        "return [values[index:index + chunk_size]\n            " \
        "for index in range(0, len(values), chunk_size)]\n\n\n" \
        "class RID_Hijack_Toolkit():",
      )
      s.gsub!(
        "numpy.array_split(raw_value.uValue, 20)",
        "split_into_chunks(raw_value.uValue)",
      )
      s.gsub!(
        "numpy.array_split(raw_value.uValue, (raw_value_length / 4))",
        "split_into_chunks(raw_value.uValue)",
      )
    end
    ...
    libexec.install "wmiexec-pro.py", "lib", "images"

    (bin/"wmiexec-pro").write <<~SH
      #!/bin/bash
      export PYTHONPATH="#{venv.site_packages}:#{libexec}"
      exec "#{venv.root}/bin/python3" "#{libexec}/wmiexec-pro.py" "$@"
    SH
  end

  test do
    output = shell_output("#{bin}/wmiexec-pro -h")
    assert_match "wmiexec-pro.py", output
  end
end
